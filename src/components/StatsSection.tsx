...
export default function StatsSection({ stats, title, description }: StatsSectionProps) {
  return (
    <section id="stats" className="section-pad blueprint-surface relative overflow-hidden border-y border-card-border">
      <div className="site-shell relative z-10 grid items-center gap-14 lg:grid-cols-[0.82fr_1.18fr] lg:gap-20">
        ...
        >
          ...
          <div className="relative mt-12 hidden aspect-[5/3] max-w-lg overflow-hidden rounded-2xl border border-card-border bg-[#07111d] lg:block" aria-hidden="true">
            ...
            <svg viewBox="0 0 560 330" className="absolute inset-0 size-full opacity-80">
              <path d="M35 220 C130 80 205 270 300 125 S455 95 530 42" fill="none" stroke="rgba(102,185,232,.48)" strokeWidth="1.5" />
              <path d="M20 270 C120 240 160 100 270 175 S420 285 540 215" fill="none" stroke="rgba(243,112,33,.35)" strokeWidth="1.5" />
              {[80, 180, 280, 390, 495].map((x, index) => (
                <g key={x}>
                  <circle cx={x} cy={[190, 112, 184, 126, 72][index]} r="8" fill={index % 2 ? '#f37021' : '#2385c1'} />
                  <circle cx={x} cy={[190, 112, 184, 126, 72][index]} r="18" fill="none" stroke="rgba(255,255,255,.16)" />
                </g>
              ))}
            </svg>
            <p className="absolute bottom-5 left-6 text-xs font-semibold uppercase tracking-[0.18em] text-[#91a8ba]">Learning signal / LHU</p>
          </div>
        </motion.div>
        ...
        {stats.length > 0 ? (
          <div className="grid grid-cols-2 border-l border-t border-card-border">
            {stats.map((stat, index) => (
              ...
              >
                <DynamicIcon name={stat.IconName} size={24} className="text-lhu-blue" />
